...
namespace Nemonuri.TransitionSystem.ExecutionFragmentSpec
...
macro_rules
  | `( 𝐸𝑥𝑒𝑐{ $ts:term }⸨ $expr:exec_expr ⸩ ) => do ``( @EvalToSet $ts $(.mk (← pure expr)) )
...
macro_rules
  | `(exec_expr| ...) => ``(ellipsis)
  | `(exec_expr| ... $expr:exec_expr ) => do ``(consEllipsis $(.mk (← pure expr)))


end Nemonuri.TransitionSystem.ExecutionFragmentSpec
