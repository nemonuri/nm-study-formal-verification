...
structure EmbedableMotiveStruct.{u} (lem: LiftableEmbedding L R) (ml: (lv: L) → Sort u) (mr: (rv: R) → Sort u) where
  ...
  compl (rv: R) (req: ¬lem.IsLiftable rv) : complType rv req

structure IsEmbedableMotive.{u} {lem: LiftableEmbedding L R} {ml: (lv: L) → Sort u} {mr: (rv: R) → Sort u} (s: lem.EmbedableMotiveStruct ml mr) : Prop where
  left_eq (lv: L) : ml lv = mr (lem lv)
  right_eq (rv: R) (req: ¬lem.IsLiftable rv) : s.complType rv req = mr rv

structure EmbedableMotive.{u} (lem: LiftableEmbedding L R) (ml: (lv: L) → Sort u) (mr: (rv: R) → Sort u)
  extends toStruct: lem.EmbedableMotiveStruct ml mr where
  valid: IsEmbedableMotive toStruct

namespace EmbedableMotive

universe u
variable {lem: LiftableEmbedding L R} {ml: (lv: L) → Sort u} {mr: (rv: R) → Sort u}

--def embedPi (em: lem.EmbedableMotive ml mr) (pi: (lv: L) → (ml lv)) (rv: R) : mr rv :=
...
/-
namespace IsEmbedableMotive

universe u
variable {lem: LiftableEmbedding L R} {ml: (lv: L) → Sort u} {mr: (rv: R) → Sort u}


--def embedRange (h: lem.IsEmbedableMotive ml mr) (lv: L) (lvr: ml lv) : mr (lem lv) :=
  --| ⟨lv, lvm⟩ => ⟨lem lv,
