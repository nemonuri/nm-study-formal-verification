...
theorem nonempty_iff_exists_getAt?_eq_some
  : (seq.toEmptyLabel = .nonempty) ↔ (∃n a, seq.getAt? n = .some a) := by
  have lm1 := seq.empty_iff_forall_getAt?_eq_none
  cases lm2: seq.toEmptyLabel
...
theorem nonempty_iff_getAt?_0_eq_some
  ...
  · have lm3 (n: ℕ) : seq.getAt? n = .none := Nat.rec lm2 (@seq.add_one_getAt?_none_of_getAt?_none) n
    have lm4 := seq.empty_iff_forall_getAt?_eq_none.mpr lm3
  ...
  · simp [Option.ne_none_iff_exists'] at lm2
    ...
    obtain ⟨a, lm2⟩ := lm2
    by_contra cont
    simp [EmptyLabel.ne_nonempty_iff_eq_empty, empty_iff_forall_getAt?_eq_none] at cont
    specialize cont 0
    simp [cont] at lm2

theorem empty_iff_getAt?_0_eq_none
  : (seq.toEmptyLabel = .empty) ↔ (seq.getAt? 0 = .none) := by
  constructor
  · intro lm1
    rewrite [empty_iff_forall_getAt?_eq_none] at lm1
    exact lm1 0
  · intro lm1
    have lm2 n := @seq.add_one_getAt?_none_of_getAt?_none _ n
    refine seq.empty_iff_forall_getAt?_eq_none.mpr ?_
    intro n
...
def toFiniteLabel (seq: Sequence α) : FiniteLabel := .ofENat seq.length?

theorem finite_iff_length?_eq_natCast
  : (seq.toFiniteLabel = .finite) ↔ ∃n, (seq.length? = Nat.cast n) := by
  dsimp [toFiniteLabel]
  exact FiniteLabel.ofENat_finite_iff_eq_natCast

theorem finite_iff_length?_lt_top
  : (seq.toFiniteLabel = .finite) ↔ (seq.length? < ⊤) := by
  dsimp [toFiniteLabel]
  exact FiniteLabel.ofENat_finite_iff_lt_top

theorem finite_iff_length?_ne_top
  : (seq.toFiniteLabel = .finite) ↔ (seq.length? ≠ ⊤) := by
  dsimp [toFiniteLabel]
  exact FiniteLabel.ofENat_finite_iff_ne_top

theorem infinite_iff_length?_eq_top
  : (seq.toFiniteLabel = .infinite) ↔ (seq.length? = ⊤) := by
  dsimp [toFiniteLabel]
  exact FiniteLabel.ofENat_infinite_iff_eq_top

theorem infinite_iff_forall_getAt?_eq_some
  : (seq.toFiniteLabel = .infinite) ↔ (∀n, ∃a, seq.getAt? n = .some a) := by
  have lm1 := @seq.length?_getAt?
  constructor
  · intro lm2
    rw [infinite_iff_length?_eq_top] at lm2
  ...
  · intro lm2
    ...
    exact infinite_iff_length?_eq_top.mpr lm2

theorem finite_iff_exists_getAt?_eq_none
  : (seq.toFiniteLabel = .finite) ↔ (∃n, seq.getAt? n = .none) := by
  have lm1 := seq.infinite_iff_forall_getAt?_eq_some
  cases lm2: seq.toFiniteLabel
  · simp [lm2] at lm1
    obtain ⟨n, lm1⟩ := lm1
...
theorem empty_and_infinite_iff_false
  : ((seq.toEmptyLabel = .empty) ∧ (seq.toFiniteLabel = .infinite)) ↔ False := by
  constructor
  · rintro ⟨lm1, lm2⟩
    rewrite [empty_iff_length?_eq_zero] at lm1
    rewrite [infinite_iff_length?_eq_top] at lm2
    simp [lm2] at lm1
  · intro lm1
    exact False.elim lm1

theorem nonempty_of_infinite (req: seq.toFiniteLabel = .infinite) : seq.toEmptyLabel = .nonempty := by
  have lm1 := fun h1 => seq.empty_and_infinite_iff_false.mp (And.intro h1 req)
  cases lm2: seq.toEmptyLabel
  · exact False.elim (lm1 lm2)
  · rfl

theorem finite_of_empty (req: seq.toEmptyLabel = .empty) : seq.toFiniteLabel = .finite := by
  have lm1 := fun h1 => seq.empty_and_infinite_iff_false.mp (And.intro req h1)
  simp [FiniteLabel.ne_infinite_iff_eq_finite] at lm1
  exact lm1

theorem finite_of_length?_eq_natCast {n: ℕ} (req: seq.length? = (Nat.cast n)) : seq.toFiniteLabel = .finite := by
  refine finite_iff_length?_eq_natCast.mpr ?_
  exists n


def head (seq: Sequence α) (req: seq.toEmptyLabel = .nonempty) : α :=
  seq[0]'(seq.nonempty_iff_length?_pos.mp req)

@[defeq]
theorem head_eq_getElem (req: seq.toEmptyLabel = .nonempty)
  : seq.head req = seq[0]'(seq.nonempty_iff_length?_pos.mp req) :=
  rfl
...
theorem head?_eq_getElem? : seq.head? = seq[0]? := by
  ...
  · simp [empty_iff_forall_getAt?_eq_none] at heq
    symm
    exact heq 0
...
def length (seq: Sequence α) (req: seq.toFiniteLabel = .finite) : ℕ :=
  seq.length?.lift (seq.finite_iff_length?_lt_top.mp req)


theorem length?_eq_natCast_length (req: seq.toFiniteLabel = .finite) : seq.length? = Nat.cast (seq.length req) := by
  dsimp [length]
  simp only [ENat.coe_lift]

@[defeq]
theorem length_eq_length?_lift (req: seq.toFiniteLabel = .finite)
  : seq.length req = seq.length?.lift (finite_iff_length?_lt_top.mp req) :=
  rfl
...
theorem length?_eq_natCast_iff_length_eq {n: ℕ}
  ...
  · intro lm1
    refine Exists.intro ?_ ?_
    · rw [finite_iff_length?_eq_natCast]
      exists n
...
theorem finite_of_length?_lt_natCast {n: ℕ} (req: seq.length? < (Nat.cast n)) : seq.toFiniteLabel = .finite := by
  induction n with
  | zero =>
    simp at req
  | succ n lm1 =>
...
theorem finite_of_length?_le_natCast {n: ℕ} (req: seq.length? ≤ (Nat.cast n)) : seq.toFiniteLabel = .finite := by
...
theorem length_eq_zero_of_length?_eq_zero (req: seq.length? = 0)
  : (seq.length (seq.empty_iff_length?_eq_zero.mpr req |> finite_of_empty) = 0) := by
  have lm1 := seq.empty_iff_length?_eq_zero.mpr req
...
theorem length_le_iff_getAt?_eq_none (req: seq.toFiniteLabel = .finite) {i: ℕ}
...
theorem length_le_iff_getAt?_eq_none_at (req: seq.toFiniteLabel = .finite) (i: ℕ)
  : ((seq.length req) ≤ i) ↔ ((seq.getAt? i) = .none) :=
  @seq.length_le_iff_getAt?_eq_none _ req i

theorem lt_length_iff_getAt?_isSome (req: seq.toFiniteLabel = .finite) {i: ℕ}
...
theorem lt_length_iff_getAt?_isSome_at (req: seq.toFiniteLabel = .finite) (i: ℕ)
  : (i < (seq.length req)) ↔ ((seq.getAt? i).isSome) :=
  @seq.lt_length_iff_getAt?_isSome _ req i


theorem getAt?_length_eq_none (req: seq.toFiniteLabel = .finite) : seq.getAt? (seq.length req) = .none := by
  have lm1 := @seq.length_le_iff_getAt?_eq_none _ req (seq.length req)
  simp only [Nat.le_refl] at lm1
  refine lm1.mp ?_
  exact True.intro

theorem getAt?_length_sub_one_isSome_iff_nonempty (req: seq.toFiniteLabel = .finite)
  ...
  · simp [lm2] at lm1
    simp [lm1]
    simp only [EmptyLabel.ne_nonempty_iff_eq_empty, empty_iff_length?_eq_zero]
    rw [← ENat.coe_zero, length?_eq_natCast_length req, ENat.coe_inj]
    exact lm2
  · simp [lm2] at lm1
    simp [lm1]
    simp [nonempty_iff_exists_getAt?_eq_some]
    exists n
    simpa [Option.isSome_iff_exists] using lm1

theorem getAt?_length_sub_one_eq_none_iff_empty (req: seq.toFiniteLabel = .finite)
...
theorem getAt?_length_sub_add_one_eq_none_iff_empty (req: seq.toFiniteLabel = .finite) {n: ℕ}
...
theorem getAt?_length_sub_pos_eq_none_iff_empty (req1: seq.toFiniteLabel = .finite) {n: ℕ} (req2: 0 < n)
...
theorem length_eq_zero_of_empty (req: seq.toEmptyLabel = .empty) : (seq.length (finite_of_empty req)) = 0 := by
  have lm2 := seq.empty_iff_length?_eq_zero.mp req
  exact length_eq_zero_of_length?_eq_zero lm2

theorem length_ne_zero_of_nonempty_finite (req1: seq.toEmptyLabel = .nonempty) (req2: seq.toFiniteLabel = .finite)
  : seq.length req2 ≠ 0 := by
  rewrite [nonempty_iff_length?_ne_zero] at req1
  exact fun x => req1 (length?_eq_zero_of_length_eq_zero x)

theorem length_pos_of_nonempty_finite (req1: seq.toEmptyLabel = .nonempty) (req2: seq.toFiniteLabel = .finite)
  : 0 < seq.length req2 :=
  Nat.pos_of_ne_zero (length_ne_zero_of_nonempty_finite req1 req2)


theorem length_eq_of_getAt?_isSome_and_add_one_eq_none
  (req1: seq.toFiniteLabel = .finite) {i: ℕ} (req2: (seq.getAt? i).isSome) (req3: seq.getAt? (i + 1) = .none)
...
theorem getAt?_eq_none_and_sub_one_isSome_iff_length_eq
  (req1: seq.toEmptyLabel = .nonempty) (req2: seq.toFiniteLabel = .finite) {i: ℕ}
...
def getFromEndAt? (seq: Sequence α) (n: ℕ) (sentinel?: Option α) : Option α :=
  match lm1: seq.toFiniteLabel with
  | .infinite => .none
  | .finite =>
  match n with
  | 0 => sentinel?
  ...
  · split <;> rename_i lm2
    · intro lm3
      ...
      · have lm5 := seq.getAt?_length_sub_add_one_eq_none_iff_empty lm1 |>.mp lm3
        rewrite [empty_iff_forall_getAt?_eq_none] at lm5
        exact lm5 _
      · rfl
    · intro lm3; clear lm3
      split <;> rename_i lm3
...
theorem getFromLastAt?_eq_none_of_infinite (req: seq.toFiniteLabel = .infinite) {i: ℕ} : seq.getFromLastAt? i = .none := by
  dsimp [getFromLastAt?, getFromEndAt?]
  split <;> rename_i lm1
  · rfl
  · simp [lm1] at req
...
theorem getFromLastAt?_eq_none_of_empty (req: seq.toEmptyLabel = .empty) {i: ℕ} : seq.getFromLastAt? i = .none := by
  ...
  · have lm2 := finite_of_empty req
    simp [lm1] at lm2
  · split <;> rename_i lm2
    · have lm3 := seq.empty_iff_length?_eq_zero.mp req
...
theorem forall_getFromLastAt?_eq_none_iff_infinite_or_empty
  : (∀(i: ℕ), seq.getFromLastAt? i = .none) ↔ ((seq.toFiniteLabel = .infinite) ∨ (seq.toEmptyLabel = .empty)) := by
  constructor
  · intro lm1
...
theorem getFromLastAt?_eq_getAt? (req1: seq.toFiniteLabel = .finite) {i: ℕ} (req2: i < seq.length req1)
...
theorem getFromLastAt?_eq_of_finite (req: seq.toFiniteLabel = .finite) {i: ℕ}
...
theorem getFromLastAt?_zero_isSome_iff_finite_and_nonempty
  : (seq.getFromLastAt? 0).isSome ↔ ((seq.toFiniteLabel = .finite) ∧ (seq.toEmptyLabel = .nonempty)) := by
  constructor
  · intro lm1
    ...
    exact seq.nonempty_iff_length?_pos |>.mpr lm3
  · rintro ⟨lm1, lm2⟩
...
theorem length_le_iff_getFromLastAt?_eq_none (req: seq.toFiniteLabel = .finite) {i: ℕ}
  : (seq.length req ≤ i) ↔ (seq.getFromLastAt? i = .none) := by
  have lm1 := Iff.not (@seq.lt_length_iff_getFromLastAt?_isSome _ req i)
  simpa using lm1


def getFromLastAt (seq: Sequence α) (i: ℕ) (req1: seq.toFiniteLabel = .finite) (req2: i < (seq.length req1)) : α :=
...
theorem last?_eq_some_last (req1: seq.toEmptyLabel = .nonempty) (req2: seq.toFiniteLabel = .finite)
...
theorem length?_eq_of_infinites
  (req1: seq.toFiniteLabel = .infinite) (req2: seq2.toFiniteLabel = .infinite)
  : seq.length? = seq2.length? := by
  rewrite [infinite_iff_length?_eq_top] at req1 req2
  rw [req1, req2]

theorem length?_ne_of_toFiniteLabel_ne (req: seq.toFiniteLabel ≠ seq2.toFiniteLabel)
  : seq.length? ≠ seq2.length? := by
  cases lm1: seq2.toFiniteLabel
  · simp [lm1, FiniteLabel.ne_finite_iff_eq_infinite] at req
    rewrite [finite_iff_length?_ne_top] at lm1
    rewrite [infinite_iff_length?_eq_top] at req
    rw [req]; symm; exact lm1
  · simp [lm1, FiniteLabel.ne_infinite_iff_eq_finite] at req
    rewrite [finite_iff_length?_ne_top] at req
    rewrite [infinite_iff_length?_eq_top] at lm1
    rw [lm1]; exact req

theorem length?_eq_of_getAt?_eq_of_finites
  (req1: seq.getAt? = seq2.getAt?) (req2: seq.toFiniteLabel = .finite) (req3: seq2.toFiniteLabel = .finite)
  : seq.length? = seq2.length? := by
  rewrite [funext_iff] at req1
  simp [finite_iff_length?_eq_natCast] at req2 req3
...
theorem toFiniteLabel_eq_of_getAt?_eq (req: seq.getAt? = seq2.getAt?) : seq.toFiniteLabel = seq2.toFiniteLabel := by
  rewrite [funext_iff] at req
  cases lm1: seq2.toFiniteLabel
  · rewrite [seq2.finite_iff_exists_getAt?_eq_none] at lm1
    obtain ⟨n, lm1⟩ := lm1
    specialize req n
    rewrite [lm1] at req
    refine seq.finite_iff_exists_getAt?_eq_none.mpr ?_
    exists n
  · rewrite [seq2.infinite_iff_forall_getAt?_eq_some] at lm1
    refine seq.infinite_iff_forall_getAt?_eq_some.mpr ?_
    intro n
    specialize req n
    obtain ⟨a1, lm1⟩ := lm1 n
    rewrite [← req] at lm1
...
theorem length?_eq_of_getAt?_eq (req: seq.getAt? = seq2.getAt?) : seq.length? = seq2.length? := by
  by_cases lm1: seq.toFiniteLabel = seq2.toFiniteLabel
  · cases lm2: seq2.toFiniteLabel
    · rewrite [lm2] at lm1
      exact seq.length?_eq_of_getAt?_eq_of_finites req lm1 lm2
    · rewrite [lm2] at lm1
      exact seq.length?_eq_of_infinites lm1 lm2
  · have lm2 := toFiniteLabel_eq_of_getAt?_eq req
