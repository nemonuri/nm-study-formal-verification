...
structure Interleaving (ts1: TransitionSystem.{us1, uact_l, uap_l}) (ts2: TransitionSystem.{us2, uact_l, uap_l}) (Act: Type uact_r) (AP: Type uap_r)  where
  act: HasHUnion.Bundle ts1.Act ts2.Act Act
  ap: HasHUnion.Bundle ts1.AP ts2.AP AP
...
abbrev LeftActTypeAt (il: Interleaving ts1 ts2 Act AP) (lb: Label) : Type _ := il.act.hasHUnion.LeftTypeAt ts1.Act ts2.Act Act lb

def embedActAt (il: Interleaving ts1 ts2 Act AP) (lb: Label) (lv: il.LeftActTypeAt lb) : Act := il.act.hasHUnion.embedAt ts1.Act ts2.Act lb lv
...
@[mk_iff]
inductive Transition (il: Interleaving ts1 ts2 Act AP) : ts1.S → ts2.S → Act → ts1.S → ts2.S → Prop where
...
def toTransitionSystemActAP (il: Interleaving ts1 ts2 Act AP) : TransitionSystem.OfActAP il.toTransitionSystem.Act il.toTransitionSystem.AP := il.toTransitionSystem.toActAP

def inferAt
  (ts1: TransitionSystem.{us1, uact_l, uap_l}) (ts2: TransitionSystem.{us2, uact_l, uap_l})
  (Act: Type uact_r) (AP: Type uap_r) [HasHUnion.Bundle ts1.Act ts2.Act Act] [HasHUnion.Bundle ts1.AP ts2.AP AP]
  : Interleaving ts1 ts2 Act AP :=
  .mk inferInstance inferInstance

end Interleaving

def interleaveAt
  (Act: Type uact_r) (AP: Type uap_r) (ts1: TransitionSystem.{us1, uact_l, uap_l}) (ts2: TransitionSystem.{us2, uact_l, uap_l})
  [HasHUnion.Bundle ts1.Act ts2.Act Act] [HasHUnion.Bundle ts1.AP ts2.AP AP]
  : TransitionSystem :=
  (Interleaving.inferAt ts1 ts2 Act AP).toTransitionSystem
